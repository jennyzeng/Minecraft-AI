...
class Tintin:
    ...
    def make_prediction(self, batch_data, rec_type):
        ...
        if maj != None:
            self.agent_host.sendCommand(
                'chat {rtype} is: {label}'.format(rtype=rec_type,
    ...
    def single_run(self, world_state):
        ...
        for rec_type, model in self.models.items():
            if len(self.batch_data) == model['batch_size']:
                ...
                if maj != None:
                    ...
                    message = '{rtype} err_rate: {err_rate}'.format(rtype=rec_type, err_rate=err_rate)
    ...
    def main(self):
        ...
        for rec_type, model in self.models.items():
            err = model['err']
            err_rate = MH.array_err_rate(err)
            print '''        | {rtype}: {err_rate:10.4f}'''.format(rtype=rec_type, err_rate=err_rate)


        print '\n closing models...'

        for rec_type, model in reversed(self.models.items()):
            model['model'].close()
